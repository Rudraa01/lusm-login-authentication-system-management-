...
// ─── Context ────────────────────────────────────────────────────────────────
const AuthEasyContext = createContext(null);
 ...
 * </AuthEasyProvider>
 */
export function AuthEasyProvider({ apiKey, baseUrl, children }) {
  ...
  const signup = useCallback(async ({ email, password, name = '', phone = '' }) => {
    setAuthError(null);
    const data = await api.call('/register', 'POST', { email, password, name, phone });
    return data;
  }, [api]);
  ...
  const verifyOtp = useCallback(async ({ email, otp }) => {
    setAuthError(null);
    const data = await api.call('/verify-otp', 'POST', { email, otp });
  ...
  const resendOtp = useCallback(async ({ email }) => {
    setAuthError(null);
    const data = await api.call('/resend-otp', 'POST', { email });
    return data;
  }, [api]);
  ...
  const login = useCallback(async ({ email, password }) => {
    setAuthError(null);
    const data = await api.call('/login', 'POST', { email, password });
    if (data.success && data.data) {
      _saveSession(data.data.user, data.data.accessToken, data.data.refreshToken);
    }
    return data;
  ...
  const forgotPassword = useCallback(async ({ email }) => {
    setAuthError(null);
    const data = await api.call('/forgot-password', 'POST', { email });
    return data;
  }, [api]);
  ...
  const resetPassword = useCallback(async ({ email, otp, newPassword }) => {
    setAuthError(null);
    const data = await api.call('/reset-password', 'POST', { email, otp, newPassword });
    return data;
  }, [api]);
  ...
  return (
    <AuthEasyContext.Provider value={value}>
      {children}
    </AuthEasyContext.Provider>
  );
}
 ...
 */
export function AuthButton({ theme = 'dark', className = '', buttonText = 'Sign In' }) {
  const { login, signup, verifyOtp, resendOtp, isAuthenticated, logout, user } = useAuthEasy();
  ...
  useEffect(() => {
    if (!open) setStatus({ loading: false, error: '', success: '' });
  }, [open]);
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      setStatus({ loading: false, error: '', success: 'Verification code sent! Check your email.' });
      setMode('otp');
    } catch (err) {
      setStatus({ loading: false, error: err.message, success: '' });
    }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setStatus({ loading: false, error: '', success: 'Code resent!' });
    } catch (err) {
      setStatus({ loading: false, error: err.message, success: '' });
    }
  };
  ...
  if (isAuthenticated && user) {
    return (
      <button onClick={logout} className={className} style={btnStyle(colors)}>
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className={className} style={btnStyle(colors)}>
        {buttonText}
      </button>
      ...
      {open && (
        ...
        >
          <div style={cardStyle(colors)}>
            {/* Close button */}
            <button onClick={() => setOpen(false)} style={closeStyle(colors)}>✕</button>

            {/* Logo + Title */}
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              <img src="https://autheasy.me/logo.png" alt="AuthEasy" style={{ height: '40px', marginBottom: '12px' }} />
              <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: colors.title }}>
              ...
              <p style={{ margin: '4px 0 0', fontSize: '13px', color: colors.text }}>
                {mode === 'otp'
                  ...
                  : mode === 'login' ? 'Sign in to continue' : 'Join in seconds'}
              </p>
            </div>

            {/* Error / Success */}
            {status.error && <div style={alertStyle('error')}>{status.error}</div>}
            {status.success && <div style={alertStyle('success')}>{status.success}</div>}

            {/* Login Form */}
            {mode === 'login' && (
              <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <Field label="Email" name="email" type="email" value={form.email} onChange={handleField} colors={colors} placeholder="you@example.com" />
                <Field label="Password" name="password" type="password" value={form.password} onChange={handleField} colors={colors} placeholder="••••••••" />
                <SubmitButton loading={status.loading} colors={colors}>Sign In</SubmitButton>
                <p style={{ textAlign: 'center', fontSize: '13px', color: colors.text, margin: 0 }}>
                  Don't have an account?{' '}
                  <span onClick={() => setMode('signup')} style={{ color: colors.accent, cursor: 'pointer', fontWeight: 600 }}>Sign up</span>
                </p>
              </form>
            )}
            ...
            {mode === 'signup' && (
              <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <Field label="Name" name="name" type="text" value={form.name} onChange={handleField} colors={colors} placeholder="John Doe" />
                <Field label="Email" name="email" type="email" value={form.email} onChange={handleField} colors={colors} placeholder="you@example.com" />
                <Field label="Password" name="password" type="password" value={form.password} onChange={handleField} colors={colors} placeholder="Min 8 chars, use @ symbol" />
                <SubmitButton loading={status.loading} colors={colors}>Create Account</SubmitButton>
                <p style={{ textAlign: 'center', fontSize: '13px', color: colors.text, margin: 0 }}>
                  Already have an account?{' '}
                  <span onClick={() => setMode('login')} style={{ color: colors.accent, cursor: 'pointer', fontWeight: 600 }}>Sign in</span>
                </p>
              </form>
            )}
            ...
            {mode === 'otp' && (
              <form onSubmit={handleOtp} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <input
                  ...
                  placeholder="• • • • • •"
                  style={{ ...inputStyle(colors), textAlign: 'center', fontSize: '22px', letterSpacing: '8px' }}
                  required
                />
                <SubmitButton loading={status.loading} colors={colors}>Verify & Log In</SubmitButton>
                <p style={{ textAlign: 'center', fontSize: '13px', color: colors.text, margin: 0 }}>
                  Didn't get it?{' '}
                  <span onClick={handleResend} style={{ color: colors.accent, cursor: 'pointer', fontWeight: 600 }}>Resend code</span>
                </p>
              </form>
            )}

            {/* Powered by AuthEasy */}
            <div style={{ textAlign: 'center', marginTop: '20px', fontSize: '11px', color: colors.text, opacity: 0.6 }}>
              Secured by <a href="https://autheasy.me" target="_blank" rel="noreferrer" style={{ color: colors.accent, textDecoration: 'none', fontWeight: 600 }}>AuthEasy</a>
            </div>
          </div>
        </div>
      )}
...
const Field = ({ label, name, type, value, onChange, colors, placeholder }) => (
  <div>
    <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: colors.label, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
      {label}
    </label>
    <input
      name={name}
...
const SubmitButton = ({ loading, colors, children }) => (
